...
try:
    from RoboCam.hand_gesture import LightweightHandGesture
except Exception:
    LightweightHandGesture = None
...
class RoboCam():
    def __init__(self):
        if platform.system() == "Windows" and not os.path.isdir(pkg_resources.resource_filename(__package__, "res/")):
            ...
            os.makedirs(os.path.join(os.path.dirname(os.path.abspath(__file__)), "res/face/")) 
        ...
        self.__handGestureFlag = False
        self.__handGestureInitFlag = False
        self.__handGesture = None
    ...
    def HandGesture(self, flag:bool):
        self.__handGestureFlag = flag

    def HandGestureInit(self):
        if self.__handGestureInitFlag is True:
            return
        if LightweightHandGesture is None:
            print("HandGesture module is not available. Install mediapipe to enable landmarks.")
            return
        self.__handGesture = LightweightHandGesture()
        self.__handGestureInitFlag = True

    def __overlay_hand_gesture(self, frame):
        if self.__handGestureFlag is False:
            return
        if self.__handGestureInitFlag is False:
            self.HandGestureInit()
        if self.__handGesture is None:
            return
        try:
            det = self.__handGesture.detect(frame)
            self.__handGesture.draw_debug(frame, det)
        except Exception:
            return
        ...
        while self.__cameraStreamFlag:
            ...
            try:
                ...
                if a != -1 and b != -1:
                    ...
                    self.__overlay_hand_gesture(frame)
    ...
    def FaceCapture(self, name:str, captureCount:int=5, path:str=None):
        if path is None:
            import os
            path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "res", "face")
    ...
    def TrainFaceData(self, facePath:str = None):
        if facePath is None:
            facePath = os.path.join(os.path.dirname(os.path.abspath(__file__)), "res", "face")
        # print(facePath)
        if platform.system() != "Windows":
    ...
    def DeleteFaceData(self, name:str, facePath:str=None):
        if facePath is None:
            import os
            facePath = os.path.join(os.path.dirname(os.path.abspath(__file__)), "res", "face")
        if platform.system() != "Windows":
    ...
    def GetArucoId(self) -> int:
        if self.__arucoDetectedIds is None or len(self.__arucoDetectedIds) == 0:
            return
        else:
    ...
    def SketchCapture(self, name:str, captureCount:int=5, path:str=None):
        if path is None:
            import os
            path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "res", "sketch")
        if platform.system() != "Windows":
            path=path.replace("\\","/")
        if bool(name) == False:
            print("Name parameter is Empty.")
    ...
    def TrainSketchData(self, sketchPath:str = None):
        if sketchPath is None:
            import os
            sketchPath = os.path.join(os.path.dirname(os.path.abspath(__file__)), "res", "sketch")
        if platform.system() != "Windows":
    ...
    def DeleteSketchData(self, name:str, sketchPath:str=None):
        if sketchPath is None:
            import os
            sketchPath = os.path.join(os.path.dirname(os.path.abspath(__file__)), "res", "sketch")
        if platform.system() != "Windows":
